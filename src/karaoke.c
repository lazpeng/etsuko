...
struct etsuko_Karaoke_t {
    etsuko_UiState_t *ui;
    etsuko_Load_t load_song;
    etsuko_Load_t load_ui_font;
    etsuko_Load_t load_lyrics_font;
    etsuko_Load_t load_audio;
    etsuko_Load_t load_album_art;
    bool loaded_override;
    etsuko_Drawable_t *version_text;
    etsuko_Drawable_t *song_name_text;
    etsuko_Drawable_t *song_artist_album_text;
    etsuko_Drawable_t *elapsed_time_text;
    etsuko_Drawable_t *remaining_time_text;
    etsuko_Drawable_t *album_image;
    etsuko_Drawable_t *song_progressbar;
    etsuko_Drawable_t *play_button;
    etsuko_Drawable_t *pause_button;
    etsuko_Container_t *left_container;
    etsuko_Container_t *right_container;
    etsuko_Container_t *song_info_container;
    etsuko_Container_t *song_controls_container;
    etsuko_LyricsView_t *lyrics_view;
};
...
void karaoke_setup(etsuko_Karaoke_t *state) {
    ...
    state->elapsed_time_text = ui_make_text(state->ui,
                                       ...
                                       state->song_info_container, &(etsuko_Layout_t){0});
    ...
    state->remaining_time_text = ui_make_text(
        ...
        state->song_info_container, &(etsuko_Layout_t){.offset_x = -1, .flags = LAYOUT_ANCHOR_RIGHT_X | LAYOUT_WRAP_AROUND_X});

    // Progress bar
    state->song_progressbar =
    ...
    // Song name
    state->song_name_text = ui_make_text(state->ui,
                                    &(etsuko_Drawable_TextData_t){
                                        .text = song_get()->name,
                                        .font_type = FONT_UI,
                                        .em = 0.9,
    ...
    state->play_button =
        ui_make_image(state->ui, &(etsuko_Drawable_ImageData_t){.file_path = "assets/play.png"}, state->song_controls_container,
    ...
    state->pause_button =
        ui_make_image(state->ui, &(etsuko_Drawable_ImageData_t){.file_path = "assets/pause.png"}, state->song_controls_container,
                      &(etsuko_Layout_t){.offset_x = 0,
                                         .offset_y = 0,
                                         .width = 0.05,
                                         .flags = LAYOUT_SPECIAL_KEEP_ASPECT_RATIO | LAYOUT_CENTER | LAYOUT_PROPORTIONAL_W});
...
static void update_elapsed_text(const etsuko_Karaoke_t *state) {
    ...
    etsuko_Drawable_TextData_t *custom_data = state->elapsed_time_text->custom_data;
...
static void update_remaining_text(const etsuko_Karaoke_t *state) {
    ...
    etsuko_Drawable_TextData_t *custom_data = state->remaining_time_text->custom_data;
...
static void update_song_progressbar(const etsuko_Karaoke_t *state) {
    if ( state->song_progressbar != nullptr ) {
        const double progress = audio_elapsed_time() / audio_total_time();
        ((etsuko_Drawable_ProgressBarData_t *)state->song_progressbar->custom_data)->progress = (float)progress;
    }
}

static void toggle_pause(const etsuko_Karaoke_t *state) {
    if ( audio_is_paused() ) {
        audio_resume();
...
static void update_play_pause_state(const etsuko_Karaoke_t *state) {
    const bool paused = audio_is_paused();
    state->play_button->enabled = paused;
    state->pause_button->enabled = !paused;
}

static void check_user_input(const etsuko_Karaoke_t *state) {
    ...
    events_get_mouse_position(&mouse_x, &mouse_y);
    {
        ...
        const double end_x = begin_x + state->song_info_container->bounds.w, end_y = can_y + state->song_artist_album_text->bounds.h;
