...
Drawable_t *ui_make_image(const unsigned char *bytes, const int length, const Drawable_ImageData_t *data, Container_t *container,
                          const Layout_t *layout) {
    Drawable_t *result = make_drawable(container, DRAW_TYPE_IMAGE, false);
    Drawable_ImageData_t *dup_data = dup_image_data(data);

    Texture_t *texture = render_make_image(bytes, length, dup_data->border_radius_em);
    result->bounds.w = texture->width;
    result->bounds.h = texture->height;

    result->custom_data = dup_data;
